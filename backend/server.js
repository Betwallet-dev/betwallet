...
app.post('/api/auth/register', (req, res) => {
    ...
    if (!username || !email || !password) return res.status(400).json({ success: false, error: 'Champs requis' });
...
app.get('/api/wallet/dashboard', (req, res) => {
    res.json({
        ...
        dashboard: {
            ...
            chartData: { labels: ['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'], values: [6800, 6900, 7000, 7100, 7050, 7150, 7150] }
        }
    });
});

app.post('/api/wallet/send', (req, res) => {
    res.json({ success: true, message: 'Transaction simulée', txHash: `0x${Math.random().toString(36).substring(2, 15)}` });
});

app.get('/', (req, res) => { res.sendFile(path.join(__dirname, 'frontend', 'index.html')); });

app.listen(PORT, '0.0.0.0', () => console.log(`🚀 BetWallet API sur port ${PORT}`));